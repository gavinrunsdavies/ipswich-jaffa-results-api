...
if ( ! class_exists( 'Ipswich_JAFFA_Results_WP_REST_API_Controller_V1' ) ) {
	
	require_once plugin_dir_path( __FILE__ ) .'class-ipswich-jaffa-results-data-access.php';
	
	class Ipswich_JAFFA_Results_WP_REST_API_Controller_V1 {
		...
		public function rest_api_init( ) {			
			...
			$namespace = 'ipswich-jaffa-api/v1'; // base endpoint for our custom API
		...
		private function register_routes_distances($namespace) {			
			...
			// http://ipswichjaffa.org.uk/wp-json/ipswich-jaffa-api/distances
		...
		public function get_races( WP_REST_Request $request ) {
			...
			$response = $this->data_access->getRaces($parameters['eventId']);
