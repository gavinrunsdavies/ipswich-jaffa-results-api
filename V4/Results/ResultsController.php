...
namespace IpswichJAFFARunningClubAPI\V4\Results;
...
require_once IPSWICH_JAFFA_API_PLUGIN_PATH . 'V4/Constants/Distances.php';
require_once 'ResultsDataAccess.php';

use IpswichJAFFARunningClubAPI\V4\BaseController as BaseController;
use IpswichJAFFARunningClubAPI\V4\Constants\CourseTypes as CourseTypes;
use IpswichJAFFARunningClubAPI\V4\Constants\Distances as Distances;
...
class ResultsController extends BaseController implements IRoute
{
	...
	public function validateResult($result, $request, $key)
	{
		if (intval($result['eventId']) < 1) {
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid eventId value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (intval($result['runnerId']) < 1) {
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid runnerId value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (intval($result['position']) < 0) {
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid position value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (intval($result['team']) < 0) {
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid team value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (
			...
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid isGrandPrixResult value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (checkdate($date['month'], $date['day'], $date['year']) === FALSE) {
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %s has invalid date value', $key, json_encode($result)),
				array('status' => 400)
			);
		}
		...
		if (strpos($result['result'], ':') !== false) {
			...
			if ($time[0] < 0 || $time[1] < 0 || $time[2] < 0 || $time[1] > 59 || $time[2] > 59) {
				return new \WP_Error(
					'rest_invalid_param',
					sprintf('%s %s has invalid time value', $key, json_encode($result)),
					array('status' => 400)
				);
			} else {
				return true;
	...
	public function isValidResultUpdateField($value, $request, $key)
	{
		if ($value == 'info' || $value == 'position' || $value == 'result' || $value == 'grandprix' || $value == 'scoring_team' || $value == 'race_id') {
			...
			return new \WP_Error(
				'rest_invalid_param',
				sprintf('%s %d must be info or position or result or grandprix or scoring_team only.', $key, $value),
				array('status' => 400)
			);
		}
	}
